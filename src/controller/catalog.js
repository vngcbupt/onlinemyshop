...
module.exports = class extends Base {
  ...
  async indexAction() {
    ...
    const model = this.model('category');
    const data = await model.limit(10).where({parent_id: 1005000}).select();

    let currentCategory = null;
    if (categoryId) {
      currentCategory = await this.model('goods').where({'category_id': categoryId}).find();
    }
    ...
    if (currentCategory && currentCategory.id) {
      currentCategory.subCategoryList = await this.model('goods').where({'category_id': currentCategory.id}).select();
    }
